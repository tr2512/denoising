...
def inference_uformer(img_dir):
    model = Uformer(img_size=512,embed_dim=16,win_size=8,token_projection='linear',token_mlp='leff',modulator=True)
    load_checkpoint(model, torch.load("uformer_checkpoint.pkl"))
    image = cv2.imread(img_dir)
    image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB) / 255
    cuts, indexes = cut_image(image, (512, 512))
    h, w = image.shape[:2]
    outputs = []
    with torch.no_grad():
        for cut in cuts:
